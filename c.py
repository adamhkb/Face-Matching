...
def generate_presigned_url(OBS, client_method, expires_in):
    ...
    for path in cursor:
        url = obsClient.createSignedUrl( # ObsClient.createSignedUrl
            ...
            bucketName = 'smart-access',
            objectKey = path[0],
            # BucketName = smart-access / ObjectKet = path[0] / expires = 2592000
            expires = expires_in
        )
        # print(url)
        URL1.append(url)
    ...
    for path in cursor1:
        url3 = obsClient.createSignedUrl( # ObsClient.createSignedUrl
            method=client_method, # Method = GET
            bucketName = 'smart-access',
            objectKey = path[0],
            # BucketName = smart-access / ObjectKet = path[0] / expires_in = 2592000
            expires = 2592000
        )
        URL2.append(url3)
...
while True:
    ...
    METHOD = 'GET'
    EXPIRES_IN = 2592000
    generate_presigned_url(OBS = obsClient,
